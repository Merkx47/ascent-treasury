...
export function GoogleTranslate() {
  ...
  const changeLanguage = (langCode: string) => {
    ...
    localStorage.setItem("ascent_trade_language", langCode);
  ...
  useEffect(() => {
    const saved = localStorage.getItem("ascent_trade_language");
    if (saved) {
  ...
  return (
    <div className="relative lang-selector-container">
      ...
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md border-2 border-border bg-background hover:bg-accent transition-all duration-200"
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-56 py-2 bg-white dark:bg-gray-900 border-2 border-border rounded-lg shadow-2xl z-[99999] animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="px-3 py-2 border-b border-border bg-muted/50">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Select Language</p>
          </div>
          <div className="max-h-64 overflow-y-auto py-1 bg-white dark:bg-gray-900">
            {languages.map((lang) => (
              <button
                key={lang.code}
